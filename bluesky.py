# false positives may occur, no way to avoid them
...
class BlueskyChecker(QThread):
    ...
    def __init__(self, usernames, webhook_url=None, debug=False, save_to_file=True, proxies=None):
        ...
        self.request_delay = 0.2  # initial delay between requests
        self.min_delay = 0.01     # minimum allowed delay
    ...
    def check_username(self, username, rate_limited_usernames=None):
        ...
        while retries <= max_retries:
            ...
            try:
                ...
                webhook_desc = f"`{username}` [is available for **Bluesky**!](https://bsky.social/profile/{username}.bsky.social)"
                webhook_color = 0x3498db
                save_file = "available_bluesky_usernames.txt"
                headers = {
                ...
                if response.status_code == 400:
                    ...
                    self.update.emit(f"\u26a0\ufe0f [RATE LIMIT] {username}: Bluesky is rate limiting! Skipping for now.")
...
class App(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Bluesky Username Checker")
    ...
    def initUI(self):
        ...
        self.prefix_input.setPlaceholderText("e.g., sky")
        ...
        self.output_text.setStyleSheet("background-color: #1a1a1a; color: #3498db; font-family: Consolas, Monaco, monospace; padding: 10px;")
    ...
    def generate_word_usernames(self, source_url, min_len, max_len, count, prefix, suffix, pick_mode, apply_length_filter):
        ...
        for word in words:
            ...
            if len(username) > 0 and username not in seen:
                seen.add(username)
        ...
        try:
            test_data = {
                "embeds": [{
                    ...
                    "color": 0x3498db,
                    "footer": {
                        "text": "Bluesky Username Checker - Webhook Test"
                    }
                }]
            }
    ...
    def start_clicked(self):
        ...
        status_text += " on Bluesky..."
        self.status_label.setText(status_text)
        self.status_label.setStyleSheet("padding: 8px; font-weight: bold; background-color: #fff9c4; border-radius: 3px;")
        self.thread = BlueskyChecker(usernames, webhook_url, debug, save_to_file, proxies)
    ...
    def checking_finished(self):
        ...
        self.stop_button.setEnabled(False)
        self.status_label.setText(f"✅ Checking complete on Bluesky!")
        self.status_label.setStyleSheet("padding: 8px; font-weight: bold; background-color: #c8e6c9; border-radius: 3px;")
    ...
    def update_progress(self, value):
        ...
        self.status_label.setText(f"🔄 Progress: {value}/{total} ({percent}%) on Bluesky")
    ...
    def get_usernames(self):
        ...
        for line in txt.splitlines():
            u = line.strip()
            # Bluesky allows letters, numbers, underscores, and hyphens
            if u and all(c.isalnum() or c in ['_', '-'] for c in u):
                usernames.append(u)
        return usernames

    def update_ui(self):
        self.title.setText("🌌 Bluesky Username Checker")
        self.title.setStyleSheet("padding: 15px; background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #3498db, stop:1 #6dd5fa); color: white; border-radius: 5px;")
        self.info_group.setTitle("ℹ️ About Bluesky Username Checker")
        self.instruction.setText("✨ Check if Bluesky usernames are available!\n⚠️ Note: Bluesky may rate limit requests. Check responsibly.\n💾 Available usernames are saved to: available_bluesky_usernames.txt")
