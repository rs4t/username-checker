...
class Checker(QThread):
	...
	def run(self):
		...
		if self.save_to_file:
			try:
				file_handle = open("available_mastodon_usernames.txt", "a")
		...
		while retries <= max_retries:
			...
			try:
				...
				if response.status_code == 404:
					...
					self.update.emit(f"⚠️ [RATE LIMIT] {username}: Mastodon is rate limiting! Skipping for now.")
		...
		try:
			webhook_data = {
				...
				"embeds": [
					{
						"id": 487189062,
						"description": f"`{username}` [is available for **Mastodon**!](https://mastodon.social/@{username})",
						"fields": [],
						"color": 0x6364FF
					}
				],
...
class App(QMainWindow):
	def __init__(self):
		super().__init__()
		self.setWindowTitle("Mastodon Username Checker")
	...
	def initUI(self):
		...
		# Title
		title = QLabel("🐘 Mastodon Username Checker")
		title_font = QFont()
		title_font.setPointSize(16)
		title_font.setBold(True)
		title.setFont(title_font)
		title.setStyleSheet("padding: 15px; background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #6364FF, stop:1 #A6B1FF); color: white; border-radius: 5px;")
		main_layout.addWidget(title)

		# Info Section
		info_group = QGroupBox("ℹ️ About Mastodon Username Checker")
		info_group.setStyleSheet("QGroupBox { font-weight: bold; }")
		info_layout = QVBoxLayout()
		instruction = QLabel("✨ Check if Mastodon usernames are available!\n⚠️ Note: Mastodon may rate limit requests. Check responsibly.\n💾 Available usernames are saved to: available_mastodon_usernames.txt")
		...
		self.prefix_input.setPlaceholderText("e.g., masto")
		...
		self.gen_button.setStyleSheet("background-color: #6364FF; color: white; padding: 8px; font-weight: bold;")
		...
		self.output_text.setStyleSheet("background-color: #1a1a1a; color: #6364FF; font-family: Consolas, Monaco, monospace; padding: 10px;")
		...
		self.start_button.setStyleSheet("background-color: #6364FF; color: white; font-weight: bold; padding: 15px; font-size: 14px;")
		...
		self.progress_bar.setStyleSheet("QProgressBar { text-align: center; height: 25px; } QProgressBar::chunk { background-color: #6364FF; }")
		main_layout.addWidget(self.progress_bar)

		# Status Label
		self.status_label = QLabel("✅ Ready to check Mastodon usernames!")
	...
	def generate_word_usernames(self, source_url, min_len, max_len, count, prefix, suffix, pick_mode, apply_length_filter):
		...
		for word in words:
			...
			if 1 <= len(username) <= 30 and username not in seen:
				seen.add(username)
	...
	def generate_usernames(self):
		...
		if is_words_mode:
			...
			while len(generated) < count and attempts < max_attempts:
				...
				username = ''.join(c for c in username if c.isalnum() or c in ['_', '-'])
				if 1 <= len(username) <= 30 and username not in generated:
		...
		try:
			test_data = {
				"embeds": [{
					...
					"color": 0x6364FF,
					"footer": {
						"text": "Mastodon Username Checker - Webhook Test"
					}
				}]
			}
	...
	def get_usernames(self):
		...
		for line in txt.splitlines():
			u = line.strip()
			# Mastodon allows letters, numbers, underscores, hyphens, 1-30 chars
			if u and all(c.isalnum() or c in ['_', '-'] for c in u) and 1 <= len(u) <= 30:
				usernames.append(u)
